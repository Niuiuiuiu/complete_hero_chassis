...
### 新增
- **运动状态机** (`dm_joint_ctrl.c: motion_state_ctrl`)
  - 5 状态：`stop` → `start_run` → `running` → `pause_running` → `over_running`
  - 根据 M3508 轮速电机目标速度自动切换：有速度时站起，无速度时蹲下
  - 高度渐变斜坡：每步 ±0.0034，200Hz 下约 625ms 完成站起/蹲下，能实现平缓站起趴下
  - 停止 30 秒后自动降到底（`over_running`），降低待机功耗
- **`motion_state` 枚举** (`M3508.h`): 定义在电机头文件中，全局 `motion_ctrler` 实例

### 变更
- **初始化蹲姿**: `tar_h` 初始值 0.76→0.335，上电保持蹲下，由状态机决定站起
- **TIM14 回调调整**: `motion_state_ctrl` 在 `process_ctrl_para` 之前调用，顺序正确
- **功率 PID 微调**: M3508 Kp 2.8→2.7，Ki 0.09→0.11
- **代码风格**: `force_to_torque` 表达式加括号明确运算顺序
...
### 功率控制测试
- **加重底盘全系统功率测试**: 底盘加装拨弹盘、若干大弹丸及一块装甲板，全负载实测功率控制效果
  - M3508 轮速电机 + DM4340 关节电机均纳入功率预算
